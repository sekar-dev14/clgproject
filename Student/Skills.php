<?php
	...
	$sql = "select fieldname , labelname , uitype , optionvalue from module_fields where tablename = 'student_skills' order by sequence;";
	$ex  = mysql_query($sql);
	...
	$sql = "select * , id as 'recordId' from student_skills where uid = '".$session->userid."' $condition  ;";
	$ex  = mysql_query($sql);
	while($rs = mysql_fetch_assoc($ex)){
	$recordSet[] = $rs;
	}
...
?>
	...
	<div style="float:right;margin:5px;">
		<a href ="<?php echo SITE_URL."/index.php?module=Student&view=Skills&addNew=NewRecord"; ?>" class="btn btn-primary">Add New</a>&nbsp;&nbsp;&nbsp;
	...
	?>
		...
		<?php
			...
			{
				$recordSet = '';
				$sql = "select * , id as 'recordId' from student_skills where uid = '".$session->userid."' ;";
				$ex  = mysql_query($sql);
				while($rs = mysql_fetch_assoc($ex)){
					$recordSet[] = $rs;
				}
					...
					foreach($FieldList as $columnname => $FIELD){
						...
						?>
							<div style="float:right;marign:10px 15px 10px 10px;">
								<a href="<?php echo SITE_URL."/index.php?module=Student&view=Skills&recordId=$recordId"?>" class="btn btn-primary">Edit</a> .
							</div>
						<?php
						echo "</div> <hr>";
					}
